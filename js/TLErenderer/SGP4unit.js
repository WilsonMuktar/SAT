...
// [ep,inclp,nodep,argpp,mp]
            ...
            opsmode)
    {
        ...
        return new Array(ep, inclp, nodep, argpp, mp);
	//#include "debug1.cpp"
    },  // end dpper
            ...
            )
    {
        ...
        var snodm, cnodm, sinim, cosim, sinomm, cosomm, day, em, emsq, gam, rtemsq,
...
// [ em, argpm, inclm, mm, nm, nodem, dndt]
    dsinit: function(
            ...
            // return
            satrec,
            em, // variable is also an INPUT and output - SEG!!
            argpm,
            inclm,
            mm,
            nm, // variable is also an INPUT and output - SEG!!
            nodem //       // not in SGP4SatData
    ...
    {
        // return values ------------------
        var dndt; // em,nm, inclm, argpm, nodem, mm are also inputs
        ...
        var temp2 = this.getgravconst(whichconst);
        tumin = temp2[0];
        mu = temp2[1];
        ...
        return new Array(em, argpm, inclm, mm, nm, nodem, dndt);
		//#include "debug3.cpp"
    },  // end dsinit
...
// [em, argpm, inclm, mm,nodem, dndt, nm]
            ...
            )
    {
        ...
        return new Array(em, argpm, inclm, mm, nodem, dndt, nm);
...
// [ainv, ao, con42, cosio, cosio2, eccsq, omeosq, posq, rp, rteosq, sinio]
            ...
            )
    {
        ...
        // return variables ----------------
        var ainv, ao, con42, cosio, cosio2, eccsq, omeosq, posq, rp, rteosq, sinio;

        /* --------------------- local variables ------------------------ */
        var ak, d1, del, adel, po, x2o3, j2, xke,
                tumin, mu, radiusearthkm, j3, j4, j3oj2;

        // sgp4fix use old way of finding gst
        var ds70;
        var ts70, tfrac, c1, thgr70, fk5r, c1p2p, thgr, thgro;
        var twopi = 2.0 * pi;
        ...
        return new Array(ainv, ao, con42, cosio, cosio2, eccsq, omeosq, posq, rp, rteosq, sinio);
    ...
    {
        /* --------------------- local variables ------------------------ */
        var ao, ainv, con42, cosio, sinio, cosio2, eccsq,
        ...
        /* ------------------------ earth constants ----------------------- */
        // sgp4fix identify constants and allow alternate values
        var temp5 = this.getgravconst(whichconst);//, tumin, mu, radiusearthkm, xke, j2, j3, j4, j3oj2 );
        tumin = temp5[0];
        mu = temp5[1];
        ...
        var ttemp = this.initl(satn, whichconst, satrec.ecco, epoch, satrec.inclo, satrec);
        ainv = ttemp[0];
        ao = ttemp[1];
        con42 = ttemp[2];
        cosio = ttemp[3];
...
//       }
        ...
        if((omeosq >= 0.0) || (satrec.no >= 0.0))
        {
            ...
            if((2 * pi / satrec.no) >= 225.0)
            {
                ...
                var ttemp2 = this.dscom(epoch, satrec.ecco, satrec.argpo, tc, satrec.inclo, satrec.nodeo, satrec.no, satrec);
                // save ouput vars
                snodm = ttemp2[0];
                cnodm = ttemp2[1];
                sinim = ttemp2[2];
                ...
                var ttemp3 = this.dsinit(whichconst,
...
//       if(satrec.error == 0)
        var sgp4Error = this.sgp4(satrec, 0.0, r, v);  // SEG removed gravity constant passing
...
//#include "debug6.cpp"
     ...
     * @return
     */
    sgp4Prop2JD: function(satrec,jd,r,v)
    {
        var tminSinceEpoch = (jd - satrec.jdsatepoch)*24.0*60.0;
        return this.sgp4(satrec, tminSinceEpoch,r, v);
    },
    ...
    sgp4: function(
            ...
            r,v)
    {
        var am, axnl, aynl, betal, cosim, cnod,
        ...
        x2o3 = 2.0 / 3.0;
        // sgp4fix identify constants and allow alternate values
        var temp5 = this.getgravconst(satrec.gravconsttype);//, tumin, mu, radiusearthkm, xke, j2, j3, j4, j3oj2 );
        tumin = temp5[0];
        mu = temp5[1];
        radiusearthkm = temp5[2];
        xke = temp5[3];
            ...
            )
    {
        ...
        switch(whichconst)
        {
            ...
            case "wgs84":
                ...
                break;
            default:
                alert("unknown gravity option:" + whichconst + ", using wgs84");
