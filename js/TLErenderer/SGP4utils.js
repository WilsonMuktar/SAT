...
var SGP4utils = (function(){

    var pi = 3.14159265358979323846;
    var OPSMODE_AFSPC = 'a';
    var OPSMODE_IMPROVED = 'i';

	return {
		
	getOPSMODE_IMPROVED: function(){
		return OPSMODE_IMPROVED;
	},
	getOPSMODE_AFSPC: function(){
		return OPSMODE_AFSPC;
	},
	getPI: function(){
		return pi;
	},
     ...
     */
    readTLEandIniSGP4: function(satName,line1,line2,opsmode,whichconst,satrec)
    {
        var deg2rad = pi / 180.0;         //   0.0174532925199433
        var xpdotp = 1440.0 / (2.0 * pi);  // 229.1831180523293
        var sec, tumin;//, mu, radiusearthkm, xke, j2, j3, j4, j3oj2;
...
//        int cardnumb, j; // numb,
        ...
        var year = 0;
        var mon, day, hr, minute;//, nexp, ibexp;
		var temp = SGP4unit.getgravconst(whichconst);
        tumin = temp[0];
//        mu = temp[1];
...
//        j3oj2 = temp[7];
        ...
        {
            alert("Error Reading TLE line 1: "+e.toString());
        ...
        {
            alert("Error Reading TLE line 2: " + e.toString());
            satrec.tleDataOk = false;
            satrec.error = 7;
            return false;
        }
		...
		var mdhms = new MDHMS();
        mdhms = this.days2mdhms(year, satrec.epochdays);
        mon = mdhms.mon;
        day = mdhms.day;
        hr = mdhms.hr;
        minute = mdhms.minute;
        ...
        satrec.jdsatepoch = this.jday(year, mon, day, hr, minute, sec);

        // ---------------- initialize the orbit at sgp4epoch -------------------
        var result = SGP4unit.sgp4init(whichconst, opsmode, satrec.satnum,
    ...
    readLine1: function(line1,satrec)
    {
        var tleLine1 = line1; // first line
        if(!tleLine1.substring(1, 3))//.startsWith("1 "))
        {
            alert("TLE line 1 not valid first line");
        }

        // satnum
        satrec.satnum = parseFloat(tleLine1.substring(2, 7));
        // classification
        satrec.classification = tleLine1.substring(7, 8); // 1 char
        // intln designator
        satrec.intldesg = tleLine1.substring(9, 17); // should be within 8
        // epochyr
        satrec.epochyr = parseFloat(tleLine1.substring(18, 20));
        // epoch days
        ...
        {
            satrec.nddot = parseFloat(tleLine1.substring(44, 50)) / 1.0E5;
            //nexp
            satrec.nexp = parseFloat(tleLine1.substring(50, 52));
        }
        //bstar
        satrec.bstar = parseFloat(tleLine1.substring(53, 59)) / 1.0E5;
        //ibex
        satrec.ibexp = parseFloat(tleLine1.substring(59, 61));
        // these last things are not essential so just try to read them, and give a warning - but no error
        try
        {
            // num b.
            satrec.numb = parseFloat(tleLine1.substring(62, 63));
            //  elnum
            satrec.elnum = parseFloat(tleLine1.substring(64, 68));
        }
        catch(e)
        {
        ...
        // checksum
        //int checksum1 = (int) parseFloat(tleLine1.substring(68));
        // if no errors yet everything went ok
        return true;
    }, // readLine1

    readLine2: function(line2,satrec)
    {
        /* Read the second line of elements. */
        //theLine = aFile.readLine();
        var tleLine2 = line2; // second line
        if(!tleLine2.substring(1, 3))//startsWith("2 "))
        {
            alert("TLE line 2 not valid second line");
        }
        // satnum
        var satnum = parseFloat(tleLine2.substring(2, 7));
        if(satnum != satrec.satnum)
        {
            alert("Warning TLE line 2 Sat Num doesn't match line1 for sat: " + satrec.name);
        }
        // inclination
        satrec.inclo = parseFloat(tleLine2.substring(8, 17));
        // nodeo
        satrec.nodeo = parseFloat(tleLine2.substring(17, 26));
        //satrec.ecco
        satrec.ecco = parseFloat(tleLine2.substring(26, 34)) / 1.0E7;
        // satrec.argpo
        satrec.argpo = parseFloat(tleLine2.substring(34, 43));
        // satrec.mo
        satrec.mo = parseFloat(tleLine2.substring(43, 52));
        // no
...
//        // checksum
//        int checksum2 = (int) parseFloat(tleLine2.substring(68));
        return true;
    }, // readLine1
    ...
    parseFloat: function(inStr)
    {
        ...
        var num = dformat.parse(trimStr, pp);
        if(null == num)
        {
...
// returns MDHMS object with the mdhms variables
    days2mdhms: function(year,days)
    {
        ...
        mdhms.hr = Math.floor(temp);
        temp = (temp - mdhms.hr) * 60.0;
        mdhms.minute = Math.floor(temp);
   ...
   invjday: function(jd)
    {
        // return vars
        var year, mon, day, hr, minute, sec;
		var leapyrs;
        var days, tu, temp;
        ...
        return new Array(year, mon, day, hr, minute, sec);
    },  // end invjday
     ...
     */
    rv2coe: function(r,v,mu)
    {
        // return variables
        var p, a, ecc, incl, omega, argp, nu, m, arglat, truelon, lonper;
        // internal
        var undefined, small, magr, magv, magn, sme,
        ...
        return new Array(p, a, ecc, incl, omega, argp, nu, m, arglat, truelon, lonper);
    },  // end rv2coe
     ...
     *    mag           magnitude of a vector
    ---------------------------------------------------------------------------- */
    cross: function(vec1,vec2,outvec)
    {
    ...
    {
        var small, undefined, magv1, magv2, temp;
        small = 0.00000001;
        undefined = 999999.1;
        magv1 = mag(vec1);
        magv2 = mag(vec2);
